...
describe('POST /users/login ', () => {
  test('It should successfully log an existing user in', async () => {
    const response = await request(app).post('/users/login').expect('Content-Type', /json/).send({
  ...
  */
  describe('POST /users/token', () => {
    test('It should return OTP and send email to user', async () => {
      const data = await request(app).post('/users/token').expect('Content-Type', /json/).send({
        email: 'ebuka422@gmail.com',
        pin: '1111',
      });
  ...
  */
  describe('POST /users/password', () => {
    test('It should fetch a new password after getting a token', async () => {
      const response = await request(app).post('/users/token').expect('Content-Type', /json/).send({
      ...
      const data = await request(app).post('/users/password').expect('Content-Type', /json/).send({
